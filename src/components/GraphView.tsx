...
type Props = {
  tasks: Task[]; seedKeys: Set<string>; selectedTags: string[]; showCompleted: boolean; graceKeys: Set<string>;
...
export function GraphView(props: Props) {
  ...
  const layout = useMemo(() => buildGraphLayout({ tasks: props.tasks, seedKeys: props.seedKeys, showCompleted: props.showCompleted, graceKeys: props.graceKeys, positions: props.positions, collapsedKeys, compact: props.compact, viewportWidth, zoom: props.zoom, today: props.today }), [props.tasks, props.seedKeys, props.showCompleted, props.graceKeys, props.positions, collapsedKeys, props.compact, viewportWidth, props.zoom, props.today]);
  ...
  const fit = () => props.onZoom(Math.max(.25, Math.min(1, viewportWidth / layout.size.width)));
  const zoomAtPointer = useCallback((event: WheelEvent) => {
    ...
    const pointX = (viewport.scrollLeft + event.clientX - rect.left) / props.zoom;
    const pointY = (viewport.scrollTop + event.clientY - rect.top) / props.zoom;
    const next = Math.max(.25, Math.min(2, props.zoom * (event.deltaY < 0 ? 1.25 : .8)));
    props.onZoom(next);
    requestAnimationFrame(() => { viewport.scrollLeft = pointX * next - (event.clientX - rect.left); viewport.scrollTop = pointY * next - (event.clientY - rect.top); });
  }, [props]);
  useEffect(() => {
  ...
  return <div className="em-graph">
    <div className="em-graph-toolbar">
      <button onClick={props.onBack}>Back to grid</button><button onClick={() => props.onZoom(Math.max(.25, props.zoom / 1.25))}>−</button><button onClick={() => props.onZoom(1)}>{Math.round(props.zoom * 100)} %</button><button onClick={() => props.onZoom(Math.min(2, props.zoom * 1.25))}>+</button><button onClick={fit}>Fit</button>
      <button onClick={() => setAddPanel({ cell: { col: 0, row: -1 } })}>+ Task</button><button onClick={() => setCollapsedKeys(new Set())}>Expand all</button><button onClick={props.onResetAll}>Reset all positions</button>
    </div>
    <div ref={viewportRef} className="em-graph-viewport">
        ...
        <div ref={(element) => { canvasRef.current = element; setNodeRef(element); }} className="em-graph-canvas" style={{ width: layout.size.width, height: layout.size.height, transform: `scale(${props.zoom})` }} onClick={(event) => { if (event.target === event.currentTarget || (event.target instanceof SVGSVGElement && event.target.classList.contains('em-graph-edges'))) setSelectedEdge(null); }} onDoubleClick={openEmptyCell}>
          ...
          <ul className="em-graph-nodes">{layout.nodes.map((node) => {
            ...
            return <TaskCard key={node.key} task={node.task} today={props.today} graceExpiresAt={undefined} isActiveDrag={props.activeTaskId === node.key} compact={props.compact} style={{ left: point.x, top: point.y, width: geometry.w, height: geometry.h }} className={`${node.manual ? 'em-task-manual' : ''} ${(!node.inSeed && props.selectedTags.length) || isClosedStatus(node.task.status) ? 'em-task-dimmed' : ''}`} onToggle={() => props.onToggleTask(node.task)} onSetStatus={(status) => props.onSetStatus(node.task, status)} onSetDueDate={(due) => props.onSetDueDate(node.task, due)} onUpdateTask={(text, tags, options, dependencies) => props.onUpdateTask(node.task, text, tags, options, dependencies)} onOpenSource={(mode) => props.onOpenSource(node.task, mode)} onOpenLink={(link) => props.onOpenLink(node.task, link)} onMoveQuadrant={(quadrant) => props.onMoveQuadrant(node.task, quadrant)} createTagSuggest={props.createTagSuggest} extendMenu={(menu: Menu) => {
              ...
              {hasBlockers && <button className="em-graph-collapse" onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); setCollapsedKeys((current) => { const next = new Set(current); node.collapsed ? next.delete(node.key) : next.add(node.key); return next; }); }}>{node.collapsed ? `▸ ${node.hiddenCount} hidden` : '▾'}</button>}
            </TaskCard>;
          })}</ul>
          {addPanel && <div className="em-graph-add-panel" style={{ left: addPanel.cell.col * (geometry.w + GRID.gapX), top: cellToPoint(addPanel.cell, { ...geometry, gapX: GRID.gapX, gapY: GRID.gapY, topRow: layout.topRow }).y, width: geometry.w * 2 + GRID.gapX }}><AddTaskInput quadrant={addPanel.target?.quadrant ?? 'OPEN'} initialTags={initialTags} createTagSuggest={props.createTagSuggest} onCancel={() => setAddPanel(null)} onSubmit={async (input) => { if (addPanel.target && addPanel.kind) await props.onAddLinked(addPanel.target, addPanel.kind, input); else if (addPanel.cell.row < 0) await props.onAddTask(input); else await props.onAddAtCell(addPanel.cell, { ...input, quadrant: 'OPEN' }); setAddPanel(null); }} /></div>}
        </div>
      </div>
    </div>
  </div>;
